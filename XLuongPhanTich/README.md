...
rag_django/urls.py          // Nơi chứa và định dạng các url GỐC của hệ thống
    ↓
rag_app/urls.py             // Chứa các url nhánh của url gốc, file nó gọi hàm nào sẽ nhận request của  
                            // cái đó
    ↓
...
views.upload_pdf            // Tạo và Lưu pdf dưới dạng nhị phân vào thư mục data sau đó chạy RAG
    ↓
pipeline.py                 // File trung gian của rag      
    ↓
...
embedding                   // Cấu hình và trả về embedding model
    ↓
faiss                       // Chuyển các chunk từ pdf_loader thành các vector database trả về FAISS obj
    ↓
retriever                   // Biến FAISS thành retriever và định nghĩa retriever để có thể nhận câu hỏi
    ↓
llm                         // Khởi tạo Ollama
                            // Sau bước này ta gói mọi thứ thành RetrieverQA (retriver + llm + promt) để 
                            // dùng vừa search vừa để tạo sinh ra câu trả lời  (bên file pipeline)       
    ↓
rag_chain                   // Biến lưu RetrieverQA global


Ask Question                // Người dùng nhập câu hỏi
    ↓
views.ask_question          // Gọi hàm ask_question và nhận request từ frontend
    ↓
rag_chain.invoke            // Dùng invoke để hỏi và tạo ra câu trả lời và source_documents nếu bật
    ↓
FAISS search                // Sau khi nhận được câu hỏi bên trong invoke sẽ nhận câu hỏi từ retriever
                            // sau đó dùng FAISS để truy vấn -> trả về chunk kết quả -> trả cho retriever
    ↓
LLM                         // Kết hợp LLM + Promt để sinh câu trả lời
    ↓
Answer                      // Câu trả lời sẽ được return về cho frontend để hiên thị
    ↓
HTML
